...
// @desc    Récupérer tous les validateurs (Admin, Director, Validator)
// @route   GET /api/users/validators
// @access  Private
export const getValidators = async (req, res, next) => {
  try {
    const validators = await User.findAll({
      where: {
        role: {
          [Op.in]: ['admin', 'director', 'validator']
        }
      },
      attributes: ['id', 'username', 'email', 'role', 'firstName', 'lastName'] // On masque le password
    });
