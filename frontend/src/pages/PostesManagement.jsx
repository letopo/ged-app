...
const PostesManagement = () => {
  ...
  const load = async () => {
    try {
      ...
      toast.error(t("Erreur de chargement des postes"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAssign = async (code) => {
    ...
    try {
      ...
      toast.success(t('Titulaire assigné'));
      setAdding(null); setPicked('');
      load();
    } catch (e) {
      toast.error(e.response?.data?.error || t("Échec de l'assignation"));
    }
  };
  ...
  const handleRemove = async (code, userId) => {
    try {
      ...
      toast.success(t('Titulaire retiré'));
      load();
    } catch (e) {
      toast.error(e.response?.data?.error || t('Échec du retrait'));
    }
  };
  ...
  const handleCreatePoste = async () => {
    ...
    try {
      ...
      toast.success(t('Poste créé'));
      setCreating(false);
      setNewPoste({ code: '', label: '', description: '' });
      load();
    } catch (e) {
      toast.error(e.response?.data?.error || t('Échec de la création du poste'));
    }
  };
  ...
  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'var(--fg-muted)' }}><Loader className="animate-spin" /> {t('Chargement…')}</div>;
  }
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--fg)', margin: 0 }}>{t('Postes & Fonctions')}</h1>
      </div>
      <p style={{ fontSize: 13, color: 'var(--fg-muted)', marginBottom: 24 }}>
        {t("Assignez les titulaires des postes qui pilotent les circuits de validation (comptable, DG, DDS…). Plus besoin de modifier un email dans le code : changez le titulaire ici.")}
      </p>
      ...
      <div style={{ marginBottom: 20 }}>
        {creating ? (
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-3)', padding: 16, display: 'grid', gap: 10 }}>
            <input placeholder={t('Code (ex: gmao)')} value={newPoste.code}
            ...
            <input placeholder={t('Libellé (ex: Responsable GMAO)')} value={newPoste.label}
              onChange={e => setNewPoste(s => ({ ...s, label: e.target.value }))}
              style={{ height: 32, borderRadius: 'var(--radius-2)', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--fg)', fontSize: 13, padding: '0 8px' }} />
            <input placeholder={t('Description (optionnel)')} value={newPoste.description}
        ...
        {postes.map(p => (
          <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-3)', padding: 16 }}>
            ...
            <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {(p.holders || []).length === 0 ? (
                <span style={{ fontSize: 12, color: 'var(--fg-subtle)', fontStyle: 'italic' }}>{t('Aucun titulaire')}</span>
              ) : (
                (p.holders || []).map(h => (
                  <span key={h.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 8px 4px 10px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 999, fontSize: 12.5, color: 'var(--fg)' }}>
                    {fullName(h)}
                    <button onClick={() => handleRemove(p.code, h.id)} title={t('Retirer')}
