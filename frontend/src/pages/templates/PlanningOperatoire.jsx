  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div className="not-printable" style={{
        ...
        <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--fg)', marginBottom: 16, marginTop: 0 }}>{t('Informations du Planning')}</h3>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
          <div>
            <label style={labelStyle}>📅 {t('Période (Ex: du 24 au 28 novembre)')} *</label>
            <input type="text" name="periode" value={formData.periode || ''} onChange={handleChange} placeholder={t('Ex: du 24 au 28 novembre')} style={inputStyle} required />
          </div>
          <div>
            <label style={labelStyle}>🏥 {t('Service')} *</label>
            <input type="text" name="service" value={formData.service || ''} onChange={handleChange} placeholder={t('Ex: Chirurgie')} style={inputStyle} required />
          </div>
        </div>

        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h4 style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)', margin: 0 }}>{t('Interventions chirurgicales')}</h4>
            <button type="button" onClick={addLine} style={{
              ...
              <Plus size={16} /> {t('Ajouter une intervention')}
            </button>
          </div>
          ...
          <div style={{ maxHeight: 384, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {(formData.lignes || []).map((ligne, index) => (
              <div key={index} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--fg)' }}>{t('Intervention #{{num}}', { num: index + 1 })}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  <div>
                    <label style={{ ...labelStyle, fontSize: 11 }}>{t('Jour de la semaine')}</label>
                    <select value={ligne.jour || ''} onChange={(e) => handleLineChange(index, 'jour', e.target.value)} style={{ ...inputStyle, fontSize: 12, padding: '4px 8px' }}>
                      ...
                      {joursOptions.map(jour => <option key={jour} value={jour}>{t(jour)}</option>)}
                    </select>
                  </div>
                  <div style={{ gridColumn: 'span 2' }}>
                    <label style={{ ...labelStyle, fontSize: 11 }}>{t('Nom/Prénom du patient ou de la patiente')}</label>
                    <input type="text" value={ligne.nomPatient || ''} onChange={(e) => handleLineChange(index, 'nomPatient', e.target.value)} placeholder={t('Ex: AKWO PRUDENCIA')} style={{ ...inputStyle, fontSize: 12, padding: '4px 8px' }} />
                  </div>
                  <div>
                    <label style={{ ...labelStyle, fontSize: 11 }}>{t('Âge')}</label>
                    <input type="text" value={ligne.age || ''} onChange={(e) => handleLineChange(index, 'age', e.target.value)} placeholder={t('Ex: 47 ANS')} style={{ ...inputStyle, fontSize: 12, padding: '4px 8px' }} />
                  </div>
                  <div style={{ gridColumn: 'span 2' }}>
                    <label style={{ ...labelStyle, fontSize: 11 }}>{t("Nature de l'intervention chirurgicale")}</label>
                    <input type="text" value={ligne.natureIntervention || ''} onChange={(e) => handleLineChange(index, 'natureIntervention', e.target.value)} placeholder={t('Ex: HRT TOTALE INDIQUEE POUR UTERUS POLYMYOMATEUX')} style={{ ...inputStyle, fontSize: 12, padding: '4px 8px' }} />
                  </div>
                  <div>
                    <label style={{ ...labelStyle, fontSize: 11 }}>{t('Intervenant')}</label>
                    <input type="text" value={ligne.intervenant || ''} onChange={(e) => handleLineChange(index, 'intervenant', e.target.value)} placeholder={t('Ex: DR MBOUOPDA')} style={{ ...inputStyle, fontSize: 12, padding: '4px 8px' }} />
                  </div>
                  <div>
                    <label style={{ ...labelStyle, fontSize: 11 }}>{t('N° Salle')}</label>
                    <input type="text" value={ligne.numeroSalle || ''} onChange={(e) => handleLineChange(index, 'numeroSalle', e.target.value)} placeholder={t('Ex: 2')} style={{ ...inputStyle, fontSize: 12, padding: '4px 8px' }} />
                  </div>
                </div>
              </div>
            ))}
      ...
      <div ref={pdfContainerRef} style={{ background: '#ffffff', padding: 32, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', width: '297mm', minHeight: '210mm' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24, paddingBottom: 16, borderBottom: '2px solid #000' }}>
          <h1 style={{ fontSize: 20, fontWeight: 700, textTransform: 'uppercase', marginBottom: 8 }}>
            {t('PROGRAMME OPÉRATOIRE DE LA SEMAINE {{periode}}', { periode: formData.periode ? formData.periode.toUpperCase() : '' })}
          </h1>
          {formData.service && <p style={{ fontSize: 16, fontWeight: 600 }}>{t('Service')}: {formData.service}</p>}
        </div>
          ...
          <thead>
            <tr style={{ background: '#ffffff' }}>
              {['JOUR DE LA\nSEMAINE', 'NOM/PRÉNOM DU PATIENT OU\nDE LA PATIENTE', 'ÂGE', 'NATURE DE L\'INTERVENTION CHIRURGICALE', 'INTERVENANT', 'N° SALLE'].map((h, i) => (
                <th key={i} style={{ border: '2px solid #000', padding: 8, fontWeight: 700, textAlign: 'center', textTransform: 'uppercase', whiteSpace: 'pre-line' }}>{t(h)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {(formData.lignes || []).map((ligne, index) => {
              ...
              return (
                <tr key={index} style={{ background: isYellow ? '#fef08a' : '#ffffff' }}>
                  <td style={{ border: '2px solid #000', padding: 8, textAlign: 'center', verticalAlign: 'top', fontWeight: 700 }}>{ligne.jour ? t(ligne.jour) : ''}</td>
