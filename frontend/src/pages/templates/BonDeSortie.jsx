    ...
    <div ref={pdfContainerRef} style={{ background: '#ffffff', padding: 32, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', margin: '0 auto', width: '210mm', minHeight: '297mm', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>{t('ORDRE DE MALTE')}</h1>
            <p style={{ fontSize: 13, margin: '2px 0 0' }}>B.P.: 56 NJOMBE</p>
            <p style={{ fontSize: 13, margin: '2px 0 0' }}>{t('Tél.: {{tel}}', { tel: '(237) 697 09 29 92' })}</p>
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <p style={{ fontSize: 13, marginBottom: 4 }}>{t('Date')} :</p>
      ...
      {/* Titre */}
      <h2 style={{ textAlign: 'center', fontSize: 22, fontWeight: 700, marginBottom: 32 }}>{t('BON DE SORTIE')}</h2>

      {/* Nom du demandeur */}
      <div style={{ marginBottom: 24 }}>
        <label style={{ fontSize: 13, fontWeight: 600 }}>{t('Nom du demandeur')} :</label>
        <input name="nomDemandeur" value={formData.nomDemandeur || ''} onChange={handleChange} className="not-printable" style={{ ...cellIn, width: '100%', borderBottom: '2px dotted #000', padding: '4px 8px', minHeight: 28, marginTop: 4 }} placeholder={t('Nom du demandeur...')} />
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #000', fontSize: 13, marginBottom: 16 }}>
        <thead>
          <tr style={{ background: '#f3f4f6' }}>
            ...
            <th style={{ border: '1px solid #000', padding: 8, width: '40%' }}>{t('Désignation')}</th>
            <th style={{ border: '1px solid #000', padding: 8, width: '15%' }}>{t('Quantité')}</th>
            <th style={{ border: '1px solid #000', padding: 8, width: '15%' }}>{t('PU')}</th>
            <th style={{ border: '1px solid #000', padding: 8, width: '20%' }}>{t('Montant')}</th>
          </tr>
        </thead>
        <tbody>
            ...
            return (
              <tr key={index}>
                ...
                <td style={{ border: '1px solid #000', padding: 4 }}>
                  <input value={line.designation || ''} onChange={(e) => handleLineChange(index, 'designation', e.target.value)} className="not-printable" style={cellIn} placeholder={t('Désignation...')} />
                  <div className="print-only" style={{ padding: 4 }}>{line.designation}</div>
                </td>
                <td style={{ border: '1px solid #000', padding: 4 }}>
                  <input type="number" value={line.quantite || ''} onChange={(e) => handleLineChange(index, 'quantite', e.target.value)} className="not-printable" style={{ ...cellIn, textAlign: 'center' }} placeholder={t('Qté')} />
                  <div className="print-only" style={{ padding: 4, textAlign: 'center' }}>{line.quantite}</div>
                </td>
                <td style={{ border: '1px solid #000', padding: 4 }}>
                  <input type="number" value={line.pu || ''} onChange={(e) => handleLineChange(index, 'pu', e.target.value)} className="not-printable" style={{ ...cellIn, textAlign: 'right' }} placeholder={t('Prix')} />
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 24 }}>
        <div style={{ width: '33%', border: '2px solid #000' }}>
          <div style={{ background: '#f3f4f6', borderBottom: '2px solid #000', padding: 8, textAlign: 'center', fontWeight: 700 }}>{t('TOTAL')}</div>
      ...
      <div style={{ marginBottom: 32 }}>
        <label style={{ fontSize: 13, fontWeight: 600 }}>{t('Montant total en lettre')} :</label>
        <input name="montantEnLettres" value={formData.montantEnLettres || ''} onChange={handleChange} className="not-printable" style={{ ...cellIn, width: '100%', borderBottom: '2px dotted #000', padding: '4px 8px', minHeight: 28, marginTop: 4 }} placeholder={t('Montant en lettres...')} />
