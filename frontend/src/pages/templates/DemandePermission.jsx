        ...
        <div ref={pdfContainerRef} style={{ background: '#ffffff', padding: 48, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', margin: '0 auto', position: 'relative', width: '210mm', minHeight: '297mm', fontFamily: 'Arial, sans-serif', fontSize: '15px', color: '#000000' }}>
            ...
            <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
                ...
                <div style={{ textAlign: 'center' }}>
                    <span style={{ fontSize: '20px', fontWeight: 'bold', display: 'block' }}>{t('ORDRE DE MALTE')}</span>
                    <span style={{ fontSize: '18px', color: '#DC2626' }}>{t('HÔPITAL SAINT JEAN DE MALTE')}</span>
                </div>
            </header>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, marginBottom: 32 }}>
                <div>
                    <label style={{ fontWeight: 600, display: 'block', marginBottom: 4 }}>{t('NOMS et Prénom(s)')} :</label>
                    ...
                    <label style={{ fontWeight: 600, display: 'block', marginTop: 16, marginBottom: 4 }}>{t('Service')} :</label>
                ...
                <div style={{ textAlign: 'right' }}>
                    <div style={{ marginBottom: 16 }}>
                        <input name="date_lieu" value={formData.date_lieu || ''} onChange={handleChange} placeholder={t('Njombé, le ...')} className="not-printable" style={{ ...inputCls, textAlign: 'right' }} />
                        <div className="print-only" style={{ textAlign: 'right', fontWeight: 700 }}>{formData.date_lieu}</div>
                    </div>
                    <p style={{ fontWeight: 700, marginTop: 32 }}>{t('A Monsieur le Directeur Général')}</p>
                    <p>{t("De l'Hôpital Saint Jean de Malte de Njombé")}</p>
                </div>
            </div>

            {/* OBJET */}
            <div style={{ marginBottom: 24 }}>
                <span style={{ fontWeight: 700 }}>{t('Objet')} : </span>
            ...
            <div style={{ marginBottom: 32, lineHeight: 1.8 }}>
                <p style={{ marginBottom: 16 }}>{t('Monsieur,')}</p>
                <p>{t('Je viens par cette demande solliciter une permission de')} <span style={{ fontWeight: 700 }}>{t('« {{count}} jour(s) ouvrable(s) »', { count: formData.totalDays })}</span>.</p>
                ...
                <div className="not-printable" style={{ margin: '16px 0', padding: 16, background: 'var(--brand-soft)', border: '1px solid var(--brand)', borderRadius: 'var(--radius-3)' }}>
                    <h4 style={{ fontWeight: 700, color: 'var(--brand)', marginBottom: 8, fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <Calendar size={16}/> {t('Périodes (Week-ends exclus automatiquement)')}
                    </h4>
                    {formData.periods?.map((period, index) => (
                        <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700, marginLeft: 8, color: 'var(--fg-muted)', minWidth: 80 }}>
                                {t('{{count}}j ouvr.', { count: calculateBusinessDays(period.startDate, period.endDate) })}
                            </span>
                    ...
                    <button onClick={addPeriod} style={{ fontSize: 12, color: 'var(--brand)', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 500, marginTop: 8, background: 'none', border: 'none', cursor: 'pointer' }}>
                        <Plus size={16}/> {t('Ajouter une période')}
                    </button>
                </div>

                {/* PÉRIODES PDF */}
                <div className="print-only" style={{ margin: '16px 0', paddingLeft: 16, borderLeft: '4px solid #d1d5db' }}>
                    <p style={{ fontWeight: 700, textDecoration: 'underline', marginBottom: 4 }}>{t('Détail des périodes :')}</p>
                    {formData.periods?.map((period, index) => (
                        <p key={index}>
                            - {t('Du')} <span style={{ fontWeight: 700 }}>{formatDate(period.startDate)}</span> {t('au')} <span style={{ fontWeight: 700 }}>{formatDate(period.endDate)}</span>
                            <span style={{ fontStyle: 'italic', fontSize: 13 }}> {t('({{count}} jours ouvrables)', { count: calculateBusinessDays(period.startDate, period.endDate) })}</span>
                        </p>
                    ))}
                </div>
                ...
                <div className="not-printable" style={{ margin: '24px 0' }}>
                    <label style={{ display: 'block', marginBottom: 8, fontWeight: 600, color: 'var(--fg)', fontSize: 13 }}>📝 {t('Type de permission')} :</label>
                    <select name="motif" value={formData.motif || 'Personnel'} onChange={handleChange} style={selectCls}>
                        ...
                        <option value="Journée Directeur">{t('Journée Directeur')}</option>
                        <option value="Journée Major">{t('Journée Major')}</option>
                        <option value="Permission d'Urgence">{t("Permission d'Urgence")}</option>
                        <option value="Exceptionnel">{t('Permission exceptionnel')}</option>
                    </select>
                    {formData.motif === 'Exceptionnel' && (
                        <div style={{ marginTop: 8 }}>
                            <label style={{ display: 'block', marginBottom: 4, fontWeight: 600, color: 'var(--danger)', fontSize: 13 }}>{t("Préciser l'événement")} :</label>
                            <select name="motif_exceptionnel" value={formData.motif_exceptionnel || ''} onChange={handleChange} style={{ ...selectCls, borderColor: 'var(--danger)' }}>
                                ...
                                {exceptionalReasons.map(reason => <option key={reason} value={reason}>{t(reason)}</option>)}
                            </select>
                        </div>
                    )}
                </div>
                ...
                <p className="print-only" style={{ marginTop: 16 }}>
                    {t('Pour le motif suivant')} : <span style={{ fontWeight: 700, fontStyle: 'italic' }}>{t(formData.motif)}</span>
                    {formData.motif === 'Exceptionnel' && formData.motif_exceptionnel && (
                        <span> : <span style={{ textDecoration: 'underline' }}>{t(formData.motif_exceptionnel)}</span></span>
                    )}.
                </p>
                ...
                <div ref={interimRef} style={{ marginTop: 24, padding: 8, border: '1px dotted #9ca3af', background: '#f9fafb' }} className="print-only">
                    <p style={{ fontWeight: 700 }}>
                        {t('Intérim assuré par')} :
                        <span id="interim-anchor" data-anchor="true" style={{ position: 'relative' }}>
                            __________________________________________________
                        </span>
                    </p>
                    <p style={{ fontSize: 10, fontStyle: 'italic', color: '#6b7280', textAlign: 'right' }}>{t('(A remplir par le validateur / Chef de service)')}</p>
                </div>

                <p style={{ marginTop: 32 }}>{t("Dans l'attente d'une suite favorable, veuillez agréer Monsieur l'expression de mon plus profond respect.")}</p>
            </div>
