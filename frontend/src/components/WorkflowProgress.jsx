        ...
        {workflows.sort((a, b) => a.step - b.step).map((step) => (
          <div key={step.id} className="flex flex-col gap-1">
            ...
            {step.status === 'rejected' && step.comment && (
              <div className="ml-8 px-3 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
                <span className="font-semibold text-red-700 dark:text-red-400">Motif du rejet : </span>
                <span className="text-red-600 dark:text-red-300">{step.comment}</span>
              </div>
            )}
          </div>
        ))}
