...
const fmtDate = (d) => {
  if (!d) return '—';
  const dt = new Date(d);
  return dt.toLocaleDateString(currentLocale(), { day: 'numeric', month: 'short', year: 'numeric' })
    + ' · ' + dt.toLocaleTimeString(currentLocale(), { hour: '2-digit', minute: '2-digit' });
};
...
const timeAgo = (d) => {
  ...
  if (m < 60) return i18n.t('il y a {{m}}m', { m });
  const h = Math.floor(m / 60);
  if (h < 24) return i18n.t('il y a {{h}}h', { h });
  return i18n.t('il y a {{d}}j', { d: Math.floor(h / 24) });
};
    ...
    <div style={{ display: 'flex', gap: 10, paddingBottom: isLast ? 0 : 18, position: 'relative' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, paddingTop: 2 }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--fg-subtle)', marginTop: 2 }}>
          {dateStr || (status === 'active' ? t('En cours') : status === 'pending' ? t('En attente') : '')}
        </div>
      </div>
    </div>
  );
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--fg-muted)', marginTop: 3 }}>
          {[doc.service || (doc.category && t(doc.category)), timeAgo(doc.createdAt)].filter(Boolean).join(' · ')}
        </div>
      </div>
    </div>
  );
...
const DocumentViewer = ({
  ...
  const handleDownload = async () => {
    ...
    } catch { toast.error(t('Erreur lors du téléchargement')); }
  };

  const handleValidate = () => onValidate?.({ comment });
  const handleReject = () => {
    if (!rejectComment.trim()) { toast(t('Le commentaire est obligatoire pour un rejet.')); return; }
    onReject?.({ comment: rejectComment });
  };
  const handleModif = () => {
    if (!modifComment.trim()) { toast(t('Précisez les modifications demandées.')); return; }
    onReject?.({ comment: modifComment, type: 'modif' });
  };
  const onFileSelect = (e) => {
    if (e.target.files?.[0]) {
      const file = e.target.files[0];
      if (file.type !== 'application/pdf') { toast(t('Veuillez sélectionner un fichier PDF uniquement.')); return; }
  ...
  const handleMerge = async (position) => {
    ...
    } catch { toast(t("Erreur lors de l'ajout de la page.")); }
  ...
  const fileTypeLabel  = isFormResponse ? t('Formulaire') : (FILE_TYPE_LABEL[doc.fileType] || (doc.fileType?.split('/')[1]?.toUpperCase()) || t('Fichier'));
  const sizeLabel      = fmtSize(doc.fileSize);
  const pageCount      = doc.pageCount || doc.metadata?.pageCount;
  const typeDisplay    = [fileTypeLabel, sizeLabel, pageCount ? t('{{count}} page(s)', { count: pageCount }) : null].filter(Boolean).join(' · ');
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9000, display: 'flex', flexDirection: 'column', background: 'var(--surface)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderBottom: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0, minHeight: 48 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1, minWidth: 0 }}>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--fg-muted)', fontSize: 13, padding: '2px 4px', borderRadius: 'var(--radius-2)', fontWeight: 500 }}>{t('Documents')}</button>
          ...
          <span className={`ged-badge ${st.cls}`} style={{ fontSize: 10, flexShrink: 0, marginLeft: 6, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 5, height: 5, borderRadius: '50%', background: st.dot, display: 'inline-block' }} />{t(st.label)}
          </span>
        </div>
        <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
          {isFormResponse ? (
            ...
            <>
              ...
              <button onClick={() => fileInputRef.current.click()} title={t('Ajouter une page PDF')} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 32, width: 32, borderRadius: 'var(--radius-2)', border: '1px solid var(--border)', background: 'transparent', color: 'var(--fg-muted)', cursor: 'pointer' }}>
                <FilePlus size={14} />
              </button>
            </>
          )}
      ...
      <div className="docviewer-body">
        ...
        <div className="docviewer-list" style={{ display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border)', background: 'var(--surface)', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', padding: '10px 14px 0', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            {[
              { key: 'pending', label: t('À traiter'), count: pendingDocs.length },
              { key: 'all',     label: t('Tout'),       count: documents.length  },
            ].map(tab => (
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {listedDocs.length === 0
              ? <div style={{ padding: 24, textAlign: 'center', color: 'var(--fg-subtle)', fontSize: 12 }}>{t('Aucun document')}</div>
              : listedDocs.map(d => (
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, padding: '7px 16px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
                <span style={{ fontSize: 12, color: 'var(--fg-muted)', fontWeight: 500 }}>
                  {pageCount ? t('Page 1 / {{count}}', { count: pageCount }) : fileTypeLabel}
                </span>
              </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 0' }}>
            ...
            {(wfSteps.length > 0 || uploaderName) && (
              <div style={{ marginBottom: 22 }}>
                <div style={secTitle}>{t('Workflow de validation')}</div>
                <WorkflowStep stepNum={1} label={t('Soumission')} userName={uploaderName}
                  dateStr={doc.createdAt ? new Date(doc.createdAt).toLocaleDateString(currentLocale(), { day: 'numeric', month: 'short' }) + ' · ' + new Date(doc.createdAt).toLocaleTimeString(currentLocale(), { hour: '2-digit', minute: '2-digit' }) : null}
                ...
                {wfSteps.map((wf, i) => {
                  ...
                  return (
                    <WorkflowStep key={wf.id || i} stepNum={i + 2}
                      label={assigneeName || t('Validation N{{n}}', { n: i + 1 })}
                      userName={t('Validation N{{n}}', { n: i + 1 }) + (roleLabel ? ` · ${roleLabel}` : '')}
                      dateStr={wf.updatedAt && wf.status !== 'pending' ? new Date(wf.updatedAt).toLocaleDateString(currentLocale(), { day: 'numeric', month: 'short' }) + ' · ' + new Date(wf.updatedAt).toLocaleTimeString(currentLocale(), { hour: '2-digit', minute: '2-digit' }) : null}
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <MetaRow icon={FileText}  label={t('Type')}      value={typeDisplay || fileTypeLabel} />
              {doc.service   && <MetaRow icon={Settings2} label={t('Service')}   value={doc.service}   />}
              {doc.category  && <MetaRow icon={Tag}       label={t('Catégorie')} value={t(doc.category)}  />}
              {doc.reference && <MetaRow icon={Hash}      label={t('Référence')} value={doc.reference} />}
              {doc.metadata?.ocrText && <MetaRow icon={Scan} label="OCR" value={t('Indexé · {{lang}}', { lang: t(doc.language || 'français') })} />}
              <MetaRow icon={Calendar} label={t('Créé le')}    value={fmtDate(doc.createdAt)} />
              {uploaderName && <MetaRow icon={User} label={t('Soumis par')} value={uploaderName} />}
              {doc.tags?.length > 0 && (
                <MetaRow icon={Tag} label={t('Tags')}>
            ...
            {showActions && !showRejectInput && !showModifInput && (
              <div style={{ marginBottom: 16 }}>
                <div style={secTitle}>{t('Commentaire')}</div>
                <textarea value={comment} onChange={e => setComment(e.target.value)} rows={3} placeholder={t('Ajouter un commentaire avant approbation...')} style={textareaStyle} />
              </div>
            )}

            {showRejectInput && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--danger)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 5 }}><AlertTriangle size={12} /> {t('Motif du rejet')}</div>
                <textarea value={rejectComment} onChange={e => setRejectComment(e.target.value)} rows={3} placeholder={t('Expliquer le motif du rejet...')} style={textareaStyle} autoFocus />
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <button onClick={() => setShowRejectInput(false)} style={{ flex: 1, height: 32, borderRadius: 'var(--radius-2)', border: '1px solid var(--border)', background: 'transparent', color: 'var(--fg-muted)', fontSize: 12, cursor: 'pointer' }}>{t('Annuler')}</button>
                  <button onClick={handleReject} style={{ flex: 2, height: 32, borderRadius: 'var(--radius-2)', border: 'none', background: 'var(--danger)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{t('Confirmer le rejet')}</button>
                </div>
              </div>
            )}
            ...
            {showModifInput && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--warning)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 5 }}><MessageSquare size={12} /> {t('Modifications demandées')}</div>
                <textarea value={modifComment} onChange={e => setModifComment(e.target.value)} rows={3} placeholder={t('Décrire les modifications à apporter...')} style={textareaStyle} autoFocus />
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <button onClick={() => setShowModifInput(false)} style={{ flex: 1, height: 32, borderRadius: 'var(--radius-2)', border: '1px solid var(--border)', background: 'transparent', color: 'var(--fg-muted)', fontSize: 12, cursor: 'pointer' }}>{t('Annuler')}</button>
                  <button onClick={handleModif} style={{ flex: 2, height: 32, borderRadius: 'var(--radius-2)', border: 'none', background: 'var(--warning)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{t('Envoyer')}</button>
                </div>
              </div>
            )}
          </div>
          ...
          {showActions && !showRejectInput && !showModifInput && (
            <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => { setShowRejectInput(true); setShowModifInput(false); }} style={{ flex: 1, height: 34, borderRadius: 'var(--radius-2)', border: '1px solid var(--border)', background: 'transparent', color: 'var(--fg-muted)', fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('Rejeter')}</button>
                <button onClick={() => { setShowModifInput(true); setShowRejectInput(false); }} style={{ flex: 1, height: 34, borderRadius: 'var(--radius-2)', border: '1px solid var(--border)', background: 'transparent', color: 'var(--fg-muted)', fontSize: 12, cursor: 'pointer', fontWeight: 500 }}>{t('Demander modifs')}</button>
      ...
      {showMergeModal && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-4)', boxShadow: 'var(--shadow-3)', padding: 24, width: 360 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--fg)', textAlign: 'center', marginBottom: 8 }}>{t('Où insérer le document ?')}</div>
            ...
            {isMerging
              ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '16px 0' }}><Loader size={22} color="var(--brand)" className="animate-spin" /><span style={{ fontSize: 13, color: 'var(--fg-muted)' }}>{t('Fusion en cours…')}</span></div>
              : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <button onClick={() => handleMerge('before')} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, height: 40, borderRadius: 'var(--radius-2)', border: '1px solid var(--brand)', background: 'var(--brand-soft)', color: 'var(--brand)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}><ArrowUp size={16} /> {t('Insérer au DÉBUT')}</button>
                  <button onClick={() => handleMerge('after')} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, height: 40, borderRadius: 'var(--radius-2)', border: 'none', background: 'var(--brand)', color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}><ArrowDown size={16} /> {t('Insérer à la FIN')}</button>
                  <button onClick={() => { setShowMergeModal(false); setSelectedMergeFile(null); }} style={{ background: 'none', border: 'none', color: 'var(--fg-muted)', fontSize: 12, cursor: 'pointer', marginTop: 4, textAlign: 'center', textDecoration: 'underline' }}>{t('Annuler')}</button>
                </div>
            }
          </div>
        </div>
