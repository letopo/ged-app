...
export default function PHPConsultationForm({ patient, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.serviceName) { setError(t('Veuillez sélectionner un service')); return; }
    if (!form.motif.trim()) { setError(t('Le motif de consultation est obligatoire')); return; }
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <h2 style={{ fontSize: 15, fontWeight: 600, color: 'var(--fg)', margin: '0 0 2px' }}>
                {t("Enregistrer l'arrivée en consultation")}
              </h2>
        ...
        <div style={{ padding: 16 }}>
          ...
          {success && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '24px 0', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg)', margin: 0 }}>{t('Arrivée enregistrée !')}</p>
              <p style={{ fontSize: 12, color: 'var(--fg-muted)', margin: 0 }}>
                {t('{{name}} est en attente de consultation — service {{service}}', { name: patient.nom, service: form.serviceName })}
              </p>
              <p style={{ fontSize: 12, color: 'var(--success)', margin: 0 }}>
                {t('La clôture se fera depuis la liste des consultations')}
              </p>
            </div>
          )}
          ...
          {!success && (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  <label style={labelStyle}>{t('Service')} <span style={{ color: 'var(--danger)' }}>*</span></label>
              ...
              <div>
                <label style={labelStyle}>{t('Motif de consultation')} <span style={{ color: 'var(--danger)' }}>*</span></label>
                <input type="text" value={form.motif}
                  onChange={e => setForm(f => ({ ...f, motif: e.target.value }))}
                  placeholder={t('Ex: Fièvre, céphalées, douleur abdominale...')}
                  required style={inputStyle} />
              </div>

              <div>
                <label style={labelStyle}>{t('Notes')} <span style={{ color: 'var(--fg-subtle)', fontWeight: 400 }}>({t('optionnel')})</span></label>
                <textarea value={form.notes}
                  onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                  rows={2} placeholder={t('Observations supplémentaires...')}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 12px', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 'var(--radius-2)' }}>
                ...
                <p style={{ fontSize: 12, color: '#b45309', margin: 0 }}>
                  {t("La consultation reste ouverte jusqu'à clôture manuelle ou clôture automatique en fin de journée (Retour au travail).")}
                </p>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, paddingTop: 4 }}>
                ...
                >
                  ...
                  {loading ? t('Enregistrement...') : t("Enregistrer l'arrivée")}
                </button>
              </div>
            </form>
          )}
